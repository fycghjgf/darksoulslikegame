...
import { CombatLog, Player, ItemType } from '../types';
...
type AnimState = 'idle' | 'attack' | 'hit' | 'hit-heavy' | 'die' | 'cast';
...
export const BattleArena: React.FC<BattleArenaProps> = ({ p1, p2, logs, currentTurnId }) => {
  ...
  useEffect(() => {
    ...
    const isHeavy = lastLog.damage > 25;
    const isMagic = lastLog.action.includes("施放"); // Simple text check

    const attackAnim = isMagic ? 'cast' : 'attack';
    const hitAnim = isHeavy ? 'hit-heavy' : 'hit';
    
    if (isHeavy || lastLog.isCrit) {
        setScreenShake(true);
    ...
    if (isP1Attacker) {
        ...
        setTimeout(() => setP2Anim(hitAnim), 300);
        setTimeout(() => {
            setP1Anim('idle');
            setP2Anim('idle');
        }, 800);
        ...
        setTimeout(() => setP1Anim(hitAnim), 300);
        setTimeout(() => {
            setP2Anim('idle');
            setP1Anim('idle');
        }, 800);
  ...
  const getAnimClass = (anim: AnimState, isLeft: boolean) => {
    ...
    if (anim === 'hit-heavy') return 'animate-hit-heavy';
    if (anim === 'cast') return 'animate-cast';
    ...
    return (
      <div className={`flex flex-col w-full md:w-1/3 gap-4 transition-all duration-500 relative z-10`}>
        ...
        <div className="relative h-6 bg-gray-900 border border-gray-700 w-full shadow-inner">
           <div 
              className={`absolute top-0 left-0 h-full transition-all duration-300 ${hpPercent < 30 ? 'bg-red-600 animate-pulse' : 'bg-gradient-to-r from-red-900 to-souls-red'}`}
        ...
        <div className={`flex gap-1 flex-wrap ${isLeft ? 'justify-start' : 'justify-end'}`}>
            {player.inventory.slice(0, 8).map((item, i) => (
              <div key={i} className={`w-6 h-6 bg-souls-gray/50 border border-white/10 rounded flex items-center justify-center text-[10px] cursor-help ${item.type === ItemType.WEAPON ? 'border-red-900/50' : ''}`} title={item.name}>
                {item.name[0]}
              </div>
            ))}
        </div>
  ...
  return (
    <div className={`flex flex-col h-full animate-fade-in ${screenShake ? 'animate-shake-hard' : ''}`}>
      {/* Visual Arena */}
      ...
      <div className="h-48 border-t-2 border-souls-gray bg-black/80 p-4 font-serif text-sm overflow-y-auto custom-scrollbar shadow-[inset_0_0_20px_rgba(0,0,0,1)]" ref={scrollRef}>
         <div className="space-y-1">
            ...
            {logs.map((log, idx) => (
               <div key={idx} className={`flex gap-2 border-b border-white/5 pb-1 ${log.damage > 20 ? 'text-red-400 font-bold' : 'text-gray-400'}`}>
